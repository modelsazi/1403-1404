...
%% C
f = @(x,y) [y(2)
            -2/y(1)*y(2)^2+landa^2*x/y(1)+landa^2*x/y(1)^2];
x_start = 0.1;
x_end   = L;
dx = 0.001;
x = x_start:dx:x_end;
...
while err > 1e-4 % newton-rafson loop
    for i = 2:n - 1  % calculating coefficients
        j(i,i-1) = -2*y(i)*y(i+1)+2*y(i)*y(i-1)+2*y(i)^2;
        j(i,i)   = y(i+1)^2 -2*y(i+1)*y(i-1)+y(i-1)^2+ 4*y(i)*y(i+1)-12*y(i)^2+4*y(i)*y(i-1)-2*dx^2*landa^2*x(i);
        j(i,i+1) = 2*y(i)*y(i+1)-2*y(i)*y(i-1)+2*y(i)^2;
        B(i)     = -(y(i)*y(i+1)^2-2*y(i)*y(i+1)*y(i-1)+y(i)*y(i-1)^2+2*y(i)^2*y(i+1)-4*y(i)^3+2*y(i)^2*y(i-1)-2*dx^2*landa^2*x(i)*y(i)+2*dx^2*landa^2*x(i));
    end
